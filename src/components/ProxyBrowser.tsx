...
export const ProxyBrowser = () => {
  ...
  const loadUrlForTab = async (tabId: string, input: string) => {
    const url = input.startsWith("http") ? input : `https://${input}`;
    updateTab(tabId, { isLoading: true, proxyContent: "", currentUrl: url });

    try {
      const res = await fetch("/functions/proxy", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ url, region: vpnEnabled ? vpnRegion : undefined }),
      });
      ...
      updateTab(tabId, { isLoading: false, proxyContent: "<h1>Failed to load</h1>", error: "Could not load page" });
    }
  };

  const navigateInput = (input: string) => {
    if (!input) return;
    if (input.endsWith(".com") || input.startsWith("http")) {
      loadUrlForTab(activeTab, input);
    } else {
      loadUrlForTab(activeTab, `https://www.google.com/search?q=${encodeURIComponent(input)}`);
    }
  };
    ...
    <div className="flex flex-col h-screen bg-background">
      ...
      <div className="glass-morphism border-b p-3 flex items-center gap-2">
        <div className="flex gap-1">
          <Button variant="ghost" size="icon" disabled={!currentTab?.historyIndex}><ArrowLeft className="h-4 w-4" onClick={() => {}} /></Button>
          <Button variant="ghost" size="icon" disabled={currentTab?.historyIndex === undefined}><ArrowRight className="h-4 w-4" onClick={() => {}} /></Button>
          <Button variant="ghost" size="icon" disabled={!currentTab?.currentUrl}><RotateCw className="h-4 w-4" onClick={() => currentTab?.currentUrl && loadUrlForTab(activeTab, currentTab.currentUrl)} /></Button>
          <Button variant="ghost" size="icon" onClick={() => updateTab(activeTab, { url: "", proxyContent: "", currentUrl: "", title: "Navis Web" })}><Home className="h-4 w-4" /></Button>
        </div>
      ...
      <div className="flex-1 relative bg-card">
        ...
        {currentTab?.proxyContent && (
          <iframe srcDoc={currentTab.proxyContent} className="w-full h-full border-0" sandbox="allow-same-origin allow-scripts allow-forms allow-popups allow-modals" title="Proxy Browser" />
        )}
